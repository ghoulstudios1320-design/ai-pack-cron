...
if week_key:
    ...
    if not week_dir.exists():
        raise RuntimeError(
            f"WEEK_KEY folder does not exist: {week_dir}"
        )
...
if not path.exists():
    raise RuntimeError(
        f"Missing distribution manifest: {path}"
    )
...
client_count = int(
    manifest.get("client_count", 0) or 0
)
...
if client_count != len(clients):
    errors.append(
        f"client_count mismatch: "
        f"manifest={client_count}, "
        f"actual={len(clients)}"
    )

if manifest.get("notion_published_client_count") != client_count:
    errors.append(
        "Notion published count does not match client count"
    )

if manifest.get("notion_publish_failed_client_count") != 0:
    errors.append(
        "Notion publish failures detected"
    )

if manifest.get("webhook_sent_client_count") != client_count:
    errors.append(
        "Webhook sent count does not match client count"
    )
...
email_failed_count = int(
    manifest.get("email_failed_client_count", 0) or 0
)
...
if email_failed_count:
    print(
        ...
        f"mode={email_mode}, "
        f"failures={email_failed_count}"
    )
else:
    print(
        f"Email health OK: mode={email_mode}"
    )

if manifest.get("still_retry_pending_client_count") != 0:
    errors.append(
        "Retry pending clients remain"
    )
...
for client in clients:
    ...
    if client.get("delivery_status") != "confirmed":
        errors.append(
            f"{client_id}: final status is "
            f"{client.get('delivery_status')}"
        )

    if client.get("error"):
        errors.append(
            f"{client_id}: error still present: "
            f"{client.get('error')}"
        )
...
print(
    f"Clients confirmed: {client_count}"
)
```
