...
if client_count != len(clients):
    errors.append(
        f"client_count mismatch: manifest={client_count}, actual={len(clients)}"
    )
...
if email_mode == "real":
    ...
    print(
        f"Email delivery not required for health check: mode={email_mode}"
    )
...
for client in clients:
    ...
    if client.get("delivery_status") != "confirmed":
        errors.append(
            f"{client_id}: final status is {client.get('delivery_status')}"
        )

    if client.get("error"):
        errors.append(
            f"{client_id}: error still present: {client.get('error')}"
        )
